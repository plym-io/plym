...
class FaqRepository(Traced):
    ...
    async def create(self, item: FaqItem) -> dict[str, Any]:
    ...
    async def get_by_id(self, faq_id: int) -> dict[str, Any] | None:
        result = await self._session.execute(
    ...
    async def update(self, faq_id: int, item: FaqItem) -> dict[str, Any] | None:
        result = await self._session.execute(
            text(
                f"""
