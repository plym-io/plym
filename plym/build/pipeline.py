...
# Bundler inputs written to static_dir by releases before build_dir existed.
# They are mutable files with fixed names, so they must not linger in a
# directory that is served with a year-long cache lifetime.
_LEGACY_INTERMEDIATES = ("fonts.css", "prism.css", "prism.js")


async def run_build(site: SiteConfig) -> BuildArtifacts:
    for name in _LEGACY_INTERMEDIATES:
        (settings.static_dir / name).unlink(missing_ok=True)
    try:
