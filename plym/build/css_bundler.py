...
class CssBundler:
    ...
    def build(self) -> str:
        fonts = self._read(settings.build_dir / "fonts.css")
        prism = self._read(settings.build_dir / "prism.css") if self._site.prism.enabled else ""
    ...
    def prism_js(self) -> str:
        if not self._site.prism.enabled:
            return ""
        return self._read(settings.build_dir / "prism.js")
