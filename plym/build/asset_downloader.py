...
log = logging.getLogger("plym.build.assets")

_HASH_LEN = 8
_SVG_SNIFF_LIMIT = 8192
_BOM = b"\xef\xbb\xbf"


def _is_svg(data: bytes) -> bool:
    head = data[:_SVG_SNIFF_LIMIT].removeprefix(_BOM).lstrip()
...
class AssetDownloader:
    ...
    async def _favicon(
        ...
        try:
            image: Image.Image
            with cast(Image.Image, Image.open(io.BytesIO(data))) as image:
                if image.format != "ICO":
                    return None
    ...
    async def _logo(self, session: aiohttp.ClientSession, source: str | None) -> SiteAsset | None:
        ...
        if data is None:
            return None
        if _is_svg(data):
            return await self._store(data, "logo", "svg")
        try:
            with cast(Image.Image, Image.open(io.BytesIO(data))) as image:
                image.load()
    ...
    async def _store(self, data: bytes, stem: str, ext: str) -> SiteAsset:
        ...
        for stale in settings.static_dir.glob(f"{stem}-*.*"):
