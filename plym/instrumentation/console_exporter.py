...
class TreeSpanProcessor(SpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        bucket.append(span)
        if span.parent is None or span.parent.is_remote or span.parent.trace_id != trace_id:
            self._flush(trace_id)
        elif len(bucket) > _MAX_BUFFERED:
            self._spans.pop(trace_id, None)
            _LOGGER.warning(
                "trace %032x exceeded %d buffered spans and was dropped",
                trace_id,
                _MAX_BUFFERED,
            )
    ...
    def _flush(self, trace_id: int) -> None:
        ...
        for s in spans:
            if s.parent is not None and s.parent.span_id in ids:
                ...
                roots.append(s)
        lines: list[str] = []
        for root in sorted(roots, key=lambda s: cast(int, s.start_time)):
    ...
    def _render(
        ...
        kids = sorted(children.get(span.context.span_id, []), key=lambda s: cast(int, s.start_time))
        for i, kid in enumerate(kids):
            self._render(kid, children, child_prefix, i == len(kids) - 1, lines, depth + 1)

    def _header(self, span: ReadableSpan) -> str:
        ...
        if status is not None:
            bits.append(self._status_code(int(cast(str | int, status))))
        bits.append(self._duration(span))
        return "■ " + "  ".join(bits)

    def _label(self, span: ReadableSpan) -> str:
    ...
    def _name(self, span: ReadableSpan) -> str:
        attrs = span.attributes or {}
        method = attrs.get("http.request.method") or attrs.get("http.method")
        if method:
            route = (
                attrs.get("http.route") or attrs.get("url.path") or attrs.get("http.target") or ""
            )
    ...
    def _duration(self, span: ReadableSpan) -> str:
        ms = (cast(int, span.end_time) - cast(int, span.start_time)) / 1_000_000
        text = f"{ms:.1f}ms"
    ...
    def _error_suffix(self, span: ReadableSpan) -> str:
        if span.status.status_code is not StatusCode.ERROR:
            return ""
        for event in span.events:
            if event.name == "exception":
                kind = cast(Mapping[str, Any], event.attributes).get("exception.type", "error")
                return self._c(_RED, f"  ✗ {kind}")
        return self._c(_RED, "  ✗")

    def _c(self, color: str | None, text: str) -> str:
